...
pub async fn run(contract_path: &str, _tests: &str, threshold: f64, output: &str) -> Result<()> {
    ...
    if covignore_path.exists() {
        if let Ok(content) = fs::read_to_string(&covignore_path) {
            let excludes: Vec<String> = content.lines()
                .map(|s| s.trim())
    ...
    println!("\n{} HTML report generated at: {}", "✓".green(), out_dir_abs.join("tarpaulin-report.html").display());
...
fn ensure_tarpaulin_installed() -> Result<()> {
    ...
    if check.is_err() || !check.unwrap().status.success() {
        println!("{}", "cargo-tarpaulin not found. Installing... (this may take a while)".yellow().bold());
        ...
        if !install.success() {
            anyhow::bail!("Installation failed. Please install manually: cargo install cargo-tarpaulin");
        }
